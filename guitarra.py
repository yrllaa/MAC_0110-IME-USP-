...
def indice(item, lst):
    ...
    '''
    n = len(lst) 
    # Percorrer lista em busca do item 
    for i in range(n):
        if lst[i] == item:
            indice_p = i
    if item not in lst:
...
def superposicao(som1, som2):
    ...
    '''
    n = len(som1) # ou som2, tanto faz, uma vez que possuem msm comprim.
    lista_final = [] # lista resultante de som1 e som2
    # Vamos ver os caras um por um e somá-los, além de add a nova lista
    for i in range(n):
        cara1 = som1[i]
        cara2 = som2[i]
        cara_f = cara1 + cara2
...
def aplique_ks(ruido, n):
    ...
    if p >= 2:
        for i in range(p, n+p):
               som[i] = 0.996*0.5*(som[i-p] + som[i-p+1])
